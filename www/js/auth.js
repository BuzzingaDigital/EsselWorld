...
function authUser() {
    facebookConnectPlugin.Event.subscribe('auth.statusChange', handleStatusChange);
}
...
function handleStatusChange(session) {
    ...
    if (session.authResponse) {
        ...
        facebookConnectPlugin.api("me/?fields=name, picture, email, username", '',
                function(response) {
                    if (!response.error) {
                        ...
                        $.ajax({
                            type: "POST",
                            url: "http://realmediaworks.com/EsselWorld/scripts.php?id=facebook&name=" + user.name + "&email=" + user.email + "&userid=" + user.id + "&username=" + user.username + "&park=EsselWorld",
                            dataType: "json",
...
function checkUserPermissions(permissionToCheck) {
    ...
    facebookConnectPlugin.api('/fql', {q: permissionsFQLQuery},
    function(response) {
...
function promptLogin() {
    facebookConnectPlugin.login(["email"], handleStatusChange, fail);
    //facebookConnectPlugin.login(null, {scope: 'email'});
}
...
function uninstallApp() {
    facebookConnectPlugin.api('/me/permissions', 'DELETE',
            function(response) {
